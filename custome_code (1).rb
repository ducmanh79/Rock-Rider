...
class Rider
  attr_accessor :score, :image, :minus, :plus, :vel_x, :vel_y, :angle, :x, :y, :score
...
def move_right rider
  if (rider.x < SCREEN_WIDTH - 70)
    rider.x += rider.vel_x
    rider.x %= SCREEN_WIDTH - 50
  end
end
...
def update_noti(notification)
  notification.color.alpha -= 0.5
end

class RockRiderGame < (Example rescue Gosu::Window)
  def initialize
    super SCREEN_WIDTH, SCREEN_HEIGHT
    self.caption = "Rock Rider"
    @endscreen = Gosu::Image.new('media/endscreen.png')
...
def update
  ...
  def draw
    ...
    if @player.score >= 15
      @endscreen.draw_rot(220, 350, ZOrder::UI, 0)
    end 
  end
